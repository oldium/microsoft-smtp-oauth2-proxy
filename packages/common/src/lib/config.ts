import { createFilter, DEFAULT_RULES_ORDER, Filter, FilterType, RuleSet } from "./filters.ts";
...
async function computeConfig() {
    ...
    const smtpKeyFile = resolvePath(rootDir, process.env.SMTP_KEY_FILE);
    const smtpCertFile = resolvePath(rootDir, process.env.SMTP_CERT_FILE);
    const withCertificates = !!smtpKeyFile && !!smtpCertFile;
    if (withCertificates && development && !fsExistsSync(smtpKeyFile!) && !fsExistsSync(smtpCertFile!)) {
        ...
        await fs.mkdir(path.dirname(smtpKeyFile!), { recursive: true });
        await fs.writeFile(smtpKeyFile!, pems.private);
        await fs.writeFile(smtpCertFile!, pems.cert);
    }
    const smtpCertificate: Certificate =
        withCertificates
            ? {
                key: await fs.readFile(smtpKeyFile!),
                cert: await fs.readFile(smtpCertFile!),
            }
            : {};
    ...
    async function resolveListenHosts(hosts: string | null): Promise<Addresses> {
        ...
        if (listenAddresses.length == 0) {
            // @ts-expect-error single null value represents catch-all network address here
    ...
    const httpsKeyFile = resolvePath(rootDir, process.env.HTTPS_KEY_FILE);
    const httpsCertFile = resolvePath(rootDir, process.env.HTTPS_CERT_FILE);
    const withHttps = !!httpsKeyFile && !!httpsCertFile;
    if (withHttps && development && !fsExistsSync(httpsKeyFile!) && !fsExistsSync(httpsCertFile!)) {
        console.info("Generating self-signed certificate for HTTPS server...");
        const attrs = [{ name: 'commonName', value: 'localhost' }];
        // noinspection SpellCheckingInspection
        const pems = await (await import("selfsigned")).generate(attrs, { keySize: 2048 });
        await fs.writeFile(httpsKeyFile!, pems.private);
        await fs.writeFile(httpsCertFile!, pems.cert);
    }
    ...
    const db: string = resolvePath(rootDir, process.env.SQLITE_PATH || (development ? "../../data/db.sqlite" : "data/db.sqlite"));
    ...
    process.env.WEB_SMTP_SERVER = smtpOptions.server.host;
    ...
    const ui = {
        hasTls: smtpPublicTlsPorts.length > 0,
        hasStartTls: smtpPublicStartTlsPorts.length > 0,
        smtpServer: smtpOptions.server.host,
        countPorts: smtpPorts.length,
        tlsPortList: smtpPublicTlsPorts,
        startTlsPortList: smtpPublicStartTlsPorts,
    }
