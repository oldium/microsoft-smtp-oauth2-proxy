...
async function createSelfSignedCertificate(): Promise<Certificate> {
...
async function constructSmtpServer(host: string, serverType: ServerTlsType, targetHost: string, targetPort: number, targetTlsType: MockServerTlsType, greetingName: string, clientTimeoutMs?: number) {
    return new SpySmtpServer(
        ...
        serverType === ServerTlsType.Tls ? { addresses: [host], ...await createSelfSignedCertificate() } : undefined,
        serverType === ServerTlsType.StartTls ? { addresses: [host], ...await createSelfSignedCertificate() } : undefined,
        serverType === ServerTlsType.AutoTls ? { addresses: [host], ...await createSelfSignedCertificate() } : undefined,
    );
}
...
async function createMocks(serverType: ServerTlsType, mockServerType: MockServerTlsType, clientTimeoutMs?: number) {
    mockServer = new MockServer("127.1.2.11", undefined, await createSelfSignedCertificate());
    await mockServer.listen();

    smtpServer = await constructSmtpServer("127.1.2.10", serverType, "127.1.2.11", mockServer.port, mockServerType, "smtp.example.com", clientTimeoutMs);
