...
async function constructSmtpServer(host: string, tlsState: TlsState, targetHost: string, targetPort: number, targetTlsState: TlsState, greetingName: string) {
    const server = createSmtpServer(
        ...
        (tlsState === TlsState.Secure) ? { addresses: [host], ...await createSelfSignedCertificate() } : undefined,
        (tlsState === TlsState.StartTls) ? { addresses: [host], ...await createSelfSignedCertificate() } : undefined,
    );
    await server.listen();
    return server;
}
...
async function constructSmtpClient(host: string, port: number, tlsState: TlsState) {
    const client = new SMTPConnection({
        ...
        logger: true,
        debug: true,
    } satisfies (SMTPConnection.Options | {secured: boolean}));
